...
internal class Bubble(
     ...
     * 当前view 方向
     */
    private val curViewDirection: Int,
    private val mArrowWidth: Float,
    private val mCornersRadius: Float,
    ...
    fun initPath(curViewDirection: Int, arrowDirection: ArrowDirection?, path: Path, strokeWidth: Float) {
        when (arrowDirection) {
            ArrowDirection.START, ArrowDirection.START_CENTER -> if (curViewDirection == View.LAYOUT_DIRECTION_LTR) {
                if (mCornersRadius <= 0) {
                    initLeftSquarePath(mRect, path, strokeWidth)
                } else if (strokeWidth > 0 && strokeWidth > mCornersRadius) {
                    initLeftSquarePath(mRect, path, strokeWidth)
                } else {
            ...
            ArrowDirection.END, ArrowDirection.END_CENTER -> if (curViewDirection == View.LAYOUT_DIRECTION_LTR) {
                if (mCornersRadius <= 0) {
                    initRightSquarePath(mRect, path, strokeWidth)
                } else if (strokeWidth > 0 && strokeWidth > mCornersRadius) {
                    initRightSquarePath(mRect, path, strokeWidth)
    ...
    private fun initPath(arrowDirection: ArrowDirection, path: Path, strokeWidth: Float) {
        when (arrowDirection) {
            ArrowDirection.START, ArrowDirection.START_CENTER, ArrowDirection.END, ArrowDirection.END_CENTER -> initPath(
                curViewDirection, arrowDirection, path, strokeWidth
            )
    ...
    private fun initLeftRoundedPath(rect: RectF, path: Path, strokeWidth: Float) {
        ...
        path.arcTo(
            RectF(
                rect.right - mCornersRadius, rect.top + strokeWidth, rect.right - strokeWidth,
        ...
        path.arcTo(
            RectF(
                rect.right - mCornersRadius, rect.bottom - mCornersRadius,
        ...
        path.arcTo(
            RectF(
                rect.left + mArrowWidth + strokeWidth, rect.bottom - mCornersRadius,
                mCornersRadius + rect.left + mArrowWidth, rect.bottom - strokeWidth
            ), 90f, 90f
        )
        ...
        path.arcTo(
            RectF(
                rect.left + mArrowWidth + strokeWidth, rect.top + strokeWidth, mCornersRadius
                        + rect.left + mArrowWidth, mCornersRadius + rect.top
            ), 180f, 90f
        )
        path.close()
    }
    ...
    private fun initTopRoundedPath(rect: RectF, path: Path, strokeWidth: Float) {
        ...
        path.arcTo(
            RectF(
                rect.left + strokeWidth, rect.bottom - mCornersRadius,
                mCornersRadius + rect.left, rect.bottom - strokeWidth
            ), 90f, 90f
        )
        path.lineTo(rect.left + strokeWidth, rect.top + mArrowHeight + mCornersRadius + strokeWidth)
        path.arcTo(
            RectF(
                rect.left + strokeWidth, rect.top + mArrowHeight + strokeWidth, mCornersRadius
                        + rect.left, mCornersRadius + rect.top + mArrowHeight
            ), 180f, 90f
        )
        path.close()
    }
    ...
    private fun initRightRoundedPath(rect: RectF, path: Path, strokeWidth: Float) {
        ...
        path.arcTo(
            RectF(
                ...
                rect.top + strokeWidth, rect.right - mArrowWidth - strokeWidth, mCornersRadius + rect.top
            ), 270f, 90f
        )
        ...
        path.arcTo(
            RectF(
                rect.right - mCornersRadius - mArrowWidth, rect.bottom - mCornersRadius,
                rect.right - mArrowWidth - strokeWidth, rect.bottom - strokeWidth
            ), 0f, 90f
        )
        path.lineTo(rect.left + mArrowWidth + strokeWidth, rect.bottom - strokeWidth)
        path.arcTo(
        ...
        path.arcTo(
            RectF(
                rect.left + strokeWidth, rect.top + strokeWidth, mCornersRadius
                        + rect.left, mCornersRadius + rect.top
            ), 180f, 90f
        )
        path.close()
    }
    ...
    private fun initBottomRoundedPath(rect: RectF, path: Path, strokeWidth: Float) {
        ...
        path.arcTo(
            RectF(
                rect.right - mCornersRadius,
                rect.top + strokeWidth, rect.right - strokeWidth, mCornersRadius + rect.top
            ), 270f, 90f
        )
        path.lineTo(rect.right - strokeWidth, rect.bottom - mArrowHeight - mCornersRadius - strokeWidth)
        path.arcTo(
            RectF(
                rect.right - mCornersRadius, rect.bottom - mCornersRadius - mArrowHeight,
                rect.right - strokeWidth, rect.bottom - mArrowHeight - strokeWidth
            ), 0f, 90f
        )
        path.lineTo(
            rect.left + mArrowWidth + mArrowPosition - strokeWidth / 2,
        ...
        path.arcTo(
            RectF(
                rect.left + strokeWidth, rect.bottom - mCornersRadius - mArrowHeight,
                mCornersRadius + rect.left, rect.bottom - mArrowHeight - strokeWidth
            ), 90f, 90f
        )
